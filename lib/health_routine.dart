...
import 'package:flutter_project/password_edit_page.dart';
import 'package:flutter_project/profile_edit_page.dart';
...
class _CreateRoutineState extends State<CreateRoutine> {
  ...
  Future<void> saveRoutineToFirestore() async {
    ...
    // 루틴 데이터를 Firestore에 추가
      DocumentReference routineRef = await firestore.collection('user').doc(currentUserId).collection('routines').add({
        'mainname': mainnameController.text,
        'subExercises': exercises.map((exercise) {
          return {
            'name': exercise.mainname,
            'subExercises': exercise.subExercisesList.map((subExercise) {
              return {
